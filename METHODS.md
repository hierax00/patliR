...
### `network_synergy(proj, condition, disease, separation = "network", alpha = 0.05, singleton = "na")`
**Out:** per compound pair: the Menche (2015) topological separation `s_AB`, each compound's disease-proximity predicate, and the Cheng et al. (2019) class `P1`–`P6` under two proximity rules (FDR-gated `cheng_class`, sign-only `cheng_class_sign`).
...
**Two classifications.** The Cheng Fig. 2 truth table over `(separated, proximal_a, proximal_b)` — `P1` overlapping/both-proximal, **`P2` Complementary Exposure = separated & both-proximal (the only class Cheng associate with efficacy)**, `P3`/`P4` one proximal, `P5`/`P6` neither — is evaluated with two proximity predicates:

- `cheng_class` — **FDR-gated** (patliR's modification): `proximal = z_score < 0` **and** `p_adjusted < alpha` from `network_proximity()`. `alpha` is **patliR's tightening — Cheng et al. use the z-score alone and state no p-value cut.** `complementary_exposure = (cheng_class == "P2")` and `synergy_score` are built on this one.
- `cheng_class_sign` — **sign-only**, the paper's rule: `proximal = z_score < 0`. `P1` = `s_AB < 0` & two negative z; `P2` = `s_AB ≥ 0` & two; `P3`/`P4` = `s_AB < 0`/`≥ 0` & one; `P5`/`P6` = `s_AB < 0`/`≥ 0` & none.

They differ whenever a compound has `z < 0` but its BH-adjusted p does not clear `alpha`, so a result must say which rule it rests on. `both_proximal = proximal_a & proximal_b` uses the FDR-gated predicates (a missing predicate counts as `FALSE`), so `both_proximal = TRUE` always implies `proximal_a = proximal_b = TRUE`; the raw-sign counterpart is `both_negative_z = z_a < 0 & z_b < 0`. `score_threshold` is recorded on every row.

**Singletons.** `⟨d_AA⟩` needs ≥ 2 members. With < 2 STRING-mapped targets `⟨d_AA⟩` is set to 0 so `s_AB`/`separated` stay inspectable, and the row is flagged `singleton_a`/`singleton_b`; this is a convention, not a measurement (Menche's `separation.py` returns `nan`), it inflates `s_AB`, and two identical single-target sets get `s_AB = 0`, i.e. "separated". By default (`singleton = "na"`) `cheng_class`, `cheng_class_sign` and `complementary_exposure` are therefore `NA` for such pairs; `singleton = "zero"` keeps the old behaviour of classifying them from the `⟨d_AA⟩ = 0` separation (recorded in `singleton_policy`). `synergy_score` is `NA` for singleton pairs under either policy, and `plot_synergy()` never counts them as `P2`.

**BH family and the `alpha` gate.** `p_adjusted` is `network_proximity()`'s Benjamini–Hochberg value across **all (condition, compound) tests of one `network_proximity()` call** — not per pair, not per condition. The adjusted p of the i-th smallest of m p-values is `q_(i) = min_{j≥i} m·p_(j)/j`; it depends on the rank, so `m/(n_random+1)` is **not** a floor (30 tests with `n_random = 100`, all at the empirical floor `1/101`, give `q = 1/101 < 0.05` for every test). The only hard bound is `q_(i) ≥ p_(i) ≥ 1/(n_random+1)`: `network_synergy()` warns when `1/(n_random+1) ≥ alpha` (gate unreachable), and otherwise, when a condition has compounds with `z < 0` but none with `p_adjusted < alpha`, logs and reports that observed outcome (the FDR-gated classes can then only be `P5`/`P6`). A merged proximity table holding rows from calls with different family sizes triggers a warning that their `p_adjusted` values are not one coherent FDR family.

**Distance direction.** `network_proximity()`'s closest distance averages over the **compound's** targets the hop count to the nearest disease gene (drug target → nearest disease gene), as in the reference proximity toolbox used by Guney et al. (2016) and Cheng et al.; Cheng et al.'s Eq. 1 as printed averages disease gene → nearest drug target. The measure is not symmetric, so `z` is not interchangeable with a disease-side implementation.

**Interactome scope.** patliR's STRING graph at `score_threshold = 400` (default) is a **functional-association** network — it includes text-mining, co-expression and database-transferred edges — whereas Cheng et al. (2019) built their interactome from experimentally supported physical protein interactions only. So the absolute `s_AB` values and the `P1`–`P6` assignments here are **not directly comparable** to Cheng's published figures; the classification is meaningful for *relative* comparison within one patliR run, not as a cross-study effect size. Raise `score_threshold` (e.g. to 700) for a higher-confidence, sparser graph.

`separation = "jaccard"` skips STRING entirely: `s_AB` and both Cheng class columns are `NA`, and `synergy_score` is gated on `both_proximal` (now the FDR-gated predicate; earlier versions used the raw sign, today's `both_negative_z`). `target_jaccard`/`complementarity` are always on the **raw UniProt** sets in both modes; `n_targets_*_mapped` carries the STRING-mapped counts. `synergy_score` `= complementarity · (−max(z_a, z_b))`, gated on `complementary_exposure` (and non-singleton) in network mode. `complementarity`, `joint_closeness = −max(z_a, z_b)` and `synergy_score` are **ad-hoc ranking quantities** for ordering candidate pairs — not validated measures of efficacy or of pharmacological synergy.
