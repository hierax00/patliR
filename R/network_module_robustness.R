...
#' distance matrix (`igraph::distances(g, weights = NA)`) -- nodes that are
#' topologically close (few hops apart) cluster into the same module,
#' regardless of whether they are compounds or targets.
#'
#' Density clustering on integer graph distances is a rough proxy for
#' community structure (which is really about edge density within vs.
#' between groups); on a small bipartite graph with a near-tied distance
#' matrix it can collapse toward one cluster or all-noise. A
#' modularity-based option (`cluster_leiden`, or bipartite modularity) is
#' planned via the `clustering=` argument -- see `ROADMAP.md`.
#'
#' For each module, runs a
...
#' component fraction over removal steps `Q = 1..N`. Higher = more robust;
#' `0` is immediate collapse. The theoretical maximum is `(N-1)/(2N) < 0.5`
#' (Schneider's definition), reached only by a module that never fragments
#' until fully removed -- not `1`. R-index also depends strongly on module
#' size, so it is **not comparable across modules of different `n_nodes`**
#' (a 1-node module scores `0`, a 2-node one `0.25`, by arithmetic).
#'
