...
test_that("network_pathview(): ok reflects whether pathview() actually wrote the PNG, not just the absence of an R error", {
  ...
  testthat::local_mocked_bindings(
    pathview = function(gene.data, pathway.id, ...) {
      if (identical(pathway.id, "hsaOK")) file.create(paste0(pathway.id, ".pathview.png"))
      invisible(NULL)
    },
    .package = "pathview"
  )
...
test_that("network_pathview() warns when a gene score exceeds 1 before pathview()'s [0, 1] colour clamp", {
  ...
  testthat::local_mocked_bindings(
    pathview = function(gene.data, pathway.id, ...) {
      file.create(paste0(pathway.id, ".pathview.png"))
      invisible(NULL)
    },
    .package = "pathview"
  )
